...
set indexConfPath="%windir%\MsdOSModules\Scripts\indexConf.cmd"
if not exist %indexConfPath% (
	echo The 'indexConf.cmd' script wasn't found in MsdOSModules.
	if "%~1"=="" pause
	exit /b 1
)
...
:: Add only the Start Menu and MsdOSDesktop paths by default
:: The MsdOS folder is so that if the user searches for a MsdOS-modified feature, a script shows up in search
%indexConf% /include "%programdata%\Microsoft\Windows\Start Menu\Programs"
%indexConf% /include "%windir%\MsdOSDesktop"
