...
set "servicesPath=%windir%\MsdOSDesktop\6. Advanced Configuration\Services"
if not exist "%servicesPath%" (
	echo Services in MsdOS folder not found, can't continue.
	if "%*"=="" pause
	exit /b 1
)
...
echo This will reset the configuration of services in the MsdOS folder.
...
:main
%title% "Enabling services in the MsdOS folder... This might take a while."
for /f "usebackq tokens=*" %%a in (`dir /b /s "%windir%\MsdOSDesktop\6. Advanced Configuration\Services" ^| find "(default)"`) do (
	call :run "%%a"
	start /min /high /wait cmd /c "%%a" /silent
)

set "atlasOther=%windir%\MsdOSModules\Other"
set "winServices=%atlasOther%\winServices.reg"
set "atlasServices=%atlasOther%\MsdOSServices.reg"
...
echo 2) Restore a full services backup of the default MsdOS services
echo 3) Nothing
echo]
