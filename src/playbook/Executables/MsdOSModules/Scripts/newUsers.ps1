...
$atlasDesktop = "$windir\MsdOSDesktop"
$atlasModules = "$windir\MsdOSModules"

$title = 'Preparing MsdOS user settings...'

if (!(Test-Path $atlasDesktop) -or !(Test-Path $atlasModules)) {
    Write-Host "MsdOS was about to configure user settings, but its files weren't found. :(" -ForegroundColor Red
    Read-Pause
    exit 1
}
...
# Set visual effects
& "$atlasDesktop\4. Interface Tweaks\Visual Effects (Animations)\MsdOS Visual Effects (default).cmd" /silent
