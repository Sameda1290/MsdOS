...
param (
	...
	[string]$PackagesPath = "$([Environment]::GetFolderPath('Windows'))\MsdOSModules\Packages",
...
& "$windir\MsdOSModules\initPowerShell.ps1"
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($NoInteraction) {
			...
			$failedMsgTitle = 'MsdOSFailedComponentMsgBox'
			$failedMsgArgs = "/c title Finalizing Installation - MsdOS & echo Do not close this window. & schtasks /delete /tn `"$failedMsgTitle`" /f > nul & " `
		...
		if ($safeModeStatus) {
			Write-Host "Please report this to the MsdOS team, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to MsdOS." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to MsdOS." -ForegroundColor Red
...
Would you like MsdOS to restart your system into Safe Mode and try again? This process shouldn't take much time.

Please note that if you chose to disable Windows Defender, it may still remain enabled if you select 'No'. However, you can always try disabling it later in the MsdOS folder.
"@

	if ((Read-MessageBox -Title "MsdOS - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
...
function MakeRepairSource {
	...
	$srcPath = "%SystemRoot%\MsdOSModules\Packages\WinSxS"
