...
# Adds MsdOS PSModulesPath to profile for the PowerShell Extension
...
if ($psEditor.Workspace.Path -and ([Environment]::GetEnvironmentVariable('LOCALBUILD_DONT_ASK_FOR_MODULES', $userEnv) -ne "$true")) {
	...
	$description = @"
MsdOS includes some PowerShell modules by default that aren't usually recognised by the VSCode PowerShell extension.
Would you like to add to your PowerShell profile to automatically recognise these modules when developing MsdOS?`n`n
"@
...
if ([bool](Test-Path 'Env:\VSCODE_*') -and (Test-Path $workspace -EA 0) -and (Test-Path $modulesFile -EA 0)) {
	...
	if (!(Test-Path $modulePath -PathType Container)) {
		Write-Warning "Couldn't find module path specified in '$modulesFile', no MsdOS modules can be loaded."
	} else {
...
while ($true) { Get-Content -Wait -LiteralPath $a -EA 0 | Write-Output; Start-Sleep 1 }
	...
	$startYmlPath = Separator "Configuration\msdos\start.yml"
	...
	if (Test-Path $oemYmlPath -PathType Leaf) {
		...
		if ($confXml.Version -match '^(0|[1-9]\d*)(\.(0|[1-9]\d*)){0,2}$') {
			...
			$oemToReplace = 'MsdOSVersionUndefined'
